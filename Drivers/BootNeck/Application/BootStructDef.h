//#define REF_FILT_UNIT_TEST
//#define UNIT_TEST_TEST_BQUADS
/*
 * StructDef.h
 *
...
#include "..\Neck37\Application\ProjectTypes.h"
#include "..\Neck37\Application\Revisions.h"
#include "..\Neck37\Application\WhoIsThisProject.h"
#include "..\Neck37\Application\HwConfig.h"
#include "BootConstDef.h"

#include "..\Drivers\BootCANStruct.h"
#include "BootTimerArr.h"
#include "BootBITTimerAlloc.h"
#include "..\Drivers\BootLowLevel.h"
...
EXTERN_TAG short unsigned Proj_Type ;
...
struct CCBit
{
    ...
    int unsigned UnusedProfileConverged : 1 ; // !< Indication that profile has converged
    int unsigned UnusedMotionConverged : 1 ; // !< Indication that motion tracks with desired precision
    int unsigned UnusedMotorFault : 1 ;
    int unsigned UnusedQuickStop : 1 ;
    int unsigned UnusedBrakesReleaseCmd : 1 ;
    int unsigned UnusedPotRefFail : 1 ; // Failure of potentiometer reference voltage
    int unsigned UnusedLoopClosureMode : 3 ;
    int unsigned UnusedSystemMode : 3 ;
    int unsigned UnusedCurrentLimit : 1 ;
    int unsigned UnusedNoCalib : 1 ; // Calib failure
    int unsigned UnusedGyroNotReady : 1 ; // Gyro still calibrates its offset
    int unsigned UnusedRecorderWaitTrigger : 1 ;
    int unsigned UnusedRecorderActive : 1 ;
    int unsigned UnusedRecorderReady  : 1 ;
    int unsigned UnusedRefGenOn  : 1 ;
    int unsigned UnusedTRefGenOn  : 1 ;
    int unsigned UnusedIsTemperature : 1 ;
    int unsigned UnusedReferenceMode      : 3 ;
    int unsigned UnusedPdo3IsPotDiff : 1 ;
    int unsigned UnusedConfigured      : 1 ;
    int unsigned UnusedHomed : 1 ;
    int unsigned UnusedDin1 : 1 ;
    int unsigned UnusedDin2 : 1 ;
};
...
struct COutMsgBuf
{
    short unsigned MsgBuf[128] ;
    short unsigned CobIdLength [16]  ;
    short unsigned MsgPutPtr ;
    short unsigned MsgGetPtr ;
};
...
{
    float Speed               ; // Encoder counts / sec
    short unsigned encoder    ; // Always normalized to 16 bit / rev
    short unsigned OutEncoder ; // Always normalized to 16 bit / rev
    short unsigned FF         ;
    short unsigned HallsAndStop  ; // struct CHallsAndStop
...
{
    long  unsigned ExtStatusWord ; // Same format as neck control word
...
//    struct CPVT PVT ;
    ...
    short unsigned NoFsiMsgCnt ;
#ifdef SLAVE_DRIVER
    struct CInMsgBuf InMsgBuf;
    struct COutMsgBuf OutMsgBuf;
    struct COutMsgBuf *pOutMsgBufTx ;
    short FsiMsgReady ;
    union UFsiMsgFromSlave OutFsiMsg;
    //union UFsiMsg2Slave InFsiMsg;
    union UFsiMsgFromSlave  *pMsg2Tx ;
    union UFsiCmd2Intfc Cmd2Intfc ;
    short unsigned TagAndUser ;
    short unsigned WasSync ;
    short unsigned InDataCnt ;
    short unsigned AxisSelector ;
    short unsigned ActiveCommunicationAxis ;
    short unsigned bDebugFSI ;
#endif
};
...
//#pragma DATA_SECTION(RecorderBuffer, "ramgs0to3"); // Assure in DMA-accesible RAM
...
//extern unsigned long volatile RecorderBuffer[REC_BUF_LEN]; //<! Recorder buffer
