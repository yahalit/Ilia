...
switch Axis 
    case 'Neck' 
        ExpMng = struct('CurLevel',4,'nCycle',10,'WaitTime',3,'CycleTime',3,'npp',4) ; 
...
%        ExpMng = struct('R',0.38,'L',1.e-3,'CurLevel',2,'F1',100,'F2',300,'nPoints',3) ; 
        ExpMng = struct('CurLevel',3,'nCycle',10,'WaitTime',3,'CycleTime',3,'npp',4) ; 
        ExperimentSafeLimits = struct( 'OverCurrent' , 16 , 'Undervoltage' , 20  , 'Overvoltage' , 60) ; 
    case 'Steering'

        ExpMng = struct('CurLevel',2,'nCycle',10,'WaitTime',1,'CycleTime',3,'npp',2) ; 
        % Experiment parameters 
        ExperimentSafeLimits = struct( 'OverCurrent' , 6 , 'Undervoltage' , 14  , 'Overvoltage' , 60) ;         
...
% Set configuration 
if SetCfg 
    MyCgf = SaveCfg([],'StamCfg.jsn') ;  %#ok<UNRCH>
    ProgCfg('StamCfg.jsn') ; 
end 
...
xlabel( '(modulo) Per Unit Electrical angle') ; 
ylabel( 'Hall sensor raw reading') ; 
...
legend({'FW','BW'}) ; 
xlabel('Per unit electrical angle') ; 
ylabel('Encoder counts in forward and backward directions') ; 
