...
#ifdef ON_BOARD_POT
    potcase = (short unsigned) ControlPars.UseCase & ( UC_USE_POT1 | UC_USE_POT2)  ;
    if ( potcase)
    {
        if (potcase == 1)
        {
            SysState.OuterSensor.OuterPos = ClaState.Pot.Position[0] ;
        }
        else
        {
            SysState.OuterSensor.OuterPos = ClaState.Pot.PosCenter ;
        }
    }
#else
#ifndef SLAVE_DRIVER
...
#ifdef ON_BOARD_GYRO
    // Gyro
    if (SysState.Gyro.bExists  )
    {
        port  = *(short unsigned *)(GPIODATA_BASE+1) ;
        ReadGyro(port&0x4) ;
    }
#endif
