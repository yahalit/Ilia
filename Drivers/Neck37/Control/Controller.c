...
{
    float CurCmd , CurMax , target , Acmd  ;
    //long unsigned lstat ;
    ...
    if ( ( refmode == E_RefModeSpeed) || ( SysState.SusiMode == 0 ) )
    {
        ClosureMode =  __min(ClosureMode,E_LC_Speed_Mode)  ;
    }
        ...
        if ( ClosureMode >= E_LC_Torque_Mode )
        {
            ...
            { // Manual mode - take current command from the G generator
                if ( refmode == E_PosModeDebugGen )
                    ...
                    if ( (ClosureMode == E_LC_Torque_Mode ) && ( E_RefModeAnalogRef == SysState.Mot.ReferenceMode) )
                    {
                        Acmd = ClaState.Pot.PotFilt[0];
                        if ( ( Acmd < ClaControlPars.MinAnalogRef) ||  ( Acmd > ClaControlPars.MaxAnalogRef) )
                        {
                            CurCmd = 0 ;
                        }
                        else
                        {
                            Acmd = (Acmd-ClaControlPars.MinAnalogRef) / __fmax(0.001f ,ClaControlPars.MaxAnalogRef-ClaControlPars.MinAnalogRef )  ;
                            CurCmd = __fmax((ClaControlPars.MaxAnalogTrqCmd-ClaControlPars.MinAnalogTrqCmd ) * Acmd,0) + ClaControlPars.MinAnalogTrqCmd ;
                        }
                    }
                    else
                    {
...
*/
    ...
    CheckSusiMode() ;
    if ( SysState.Mot.ReferenceMode != E_RefModeAnalogRef )
    {
