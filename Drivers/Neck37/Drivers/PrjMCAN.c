...
{
    //float MeasuredMessageTime , MessageTimeInnovation, pref;
    //long  NextMessageTimePrediction ;


     SysState.MCanSupport.Usec4Sync = SysState.Timing.UsecTimer ;

    if ( (SysState.MCanSupport.PdoDirtyBoard &1) == 0 )
    {
    ...
    SysState.MCanSupport./ = SysState.MCanSupport.Usec4Sync ;
    ...
    if ( SysState.MCanSupport.SyncValid )
    {
        ...
        {
            pref = * (float *) &SysState.MCanSupport.uPDO1Rx.f[1]  ;
            if ( ( isnan(pref) == 0 ) && (__fmax(__fmin(pref,ControlPars.MaxPositionCmd),ControlPars.MinPositionCmd)== pref ))
            {
                SysState.InterpolationPosRef = pref ;
            }
#ifdef PVTEnabled
            PVNewMessageDriver(SysState.MCanSupport.uPDO1Rx.f[1] ) ;
#endif
        }
    }
    else
    {
        ...
        if (SysState.MCanSupport.PdoDirtyBoard &1)
        {
...
#ifndef ON_BOARD_CAN
inline
short unsigned GetPlaceInMessageQueue(void)
{
    short unsigned used = (( SysState.OutMsgBuf.MsgPutPtr - SysState.OutMsgBuf.MsgGetPtr) & 0xf) ;
    return 15 - used ;
}
#endif
...
struct CCanMsg TxMsg ;
#pragma FUNCTION_OPTIONS ( RtCanService, "--opt_level=3" );
...
*/
    ...
    if ( nf )
    {
        ...
        if ( dlc <= 16 )
        {
            switch ( cobId)
            {
//#ifdef SLAVE_DRIVER Yankl
            // Master devices handle NMT directly on acceptance
            case CAN_NMT:
                // NMT for us
...
//#endif
    ...
    if ( SysState.WasSync)
    {
        ...
        // Build TX PD1 batch
            // TX PD1 batch is application dependent.
            // Always:  PDO1 [0..3] = CBIT
            //          PDO1 [4..5] = 10 bit current + part of CBIT2
            //          PDO1 [6..7] = Exception report
        ...
        // PDO3 for steering: Position ref, feedback, outer, and encoder position
        TxMsg.cobId     = CanId + PDO3_TX  ;
        if  ( (ControlPars.UseCase  & PDO3_CONFIG_WHEEL ) == 0 )
        { // Steering
            u2.us[0] = Angle2Short( SysState.PosControl.FilteredPosReference ) ;
            u2.us[1] = Angle2Short( SysState.PosControl.PosFeedBack ) ;
            u3.us[0] = Angle2Short( SysState.OuterSensor.OuterPos ) ;
            u3.us[1] = Angle2Short( ClaState.Encoder1.UserPos     ) ;
            TxMsg.data[0] = u2.ul ;
            TxMsg.data[1] = u3.ul ;
            SetMsg2HW(&TxMsg ) ;
        }
    ...
    {
        SysState.BlockUpload.DelayCntr = 0 ;
    }
}

/*
 * Set Message to the FSI
 */
short SetMsg2HW(struct CCanMsg * pMsg)
{
    short unsigned next ;
