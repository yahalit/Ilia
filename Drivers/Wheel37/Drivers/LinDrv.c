...
/*
short GetFromRxBuf(short unsigned buf, short unsigned cntmax)
{
    short unsigned n , cnt ;
    short unsigned m ;
    short unsigned *ptr ;
    RxPutPtr = HWREGH(DMA_CH1_BASE + DMA_O_TRANSFER_COUNT);
    if ( RxGetPtr == RxPutPtr )
    {
        return 0 ;
    }
    n = (RxGetPtr - RxPutPtr) & (LIN_BUFFER_LEN-1) ;
    n = __min( n , cntmax) ;
    m = LIN_BUFFER_LEN-1 - n ;
    for ( cnt = 0 ; cnt < n ; cnt++ )
    {
        *ptr++ = UartRxSwBuf[m] ;
        m = (m+1) & (LIN_BUFFER_LEN-1) ;
    }
    return n ;
}
*/
...
void setupDMA(void)
{
   ...
   //                     uint32_t destWrapSize, int16_t destStep)
     ...
     */
    // Set start and end addresses
    DMA_configAddresses(DMA_CH2_BASE, (uint16_t *)(LINA_BASE + LIN_O_SCITD), (uint16_t *)UartTxSwBuf);
    ...
    // Set source and mode
    DMA_configMode(DMA_CH2_BASE, DMA_TRIGGER_LINATX, DMA_CFG_ONESHOT_DISABLE | DMA_CFG_CONTINUOUS_DISABLE | DMA_CFG_SIZE_16BIT);
...
void setupLin(void)
{
    ...
    HWREGH (LINA_BASE + LIN_O_SCIPIO0) = 6 ; // Enable TX and RX pins

    HWREG_BP (LINA_BASE + LIN_O_SCISETINT) = 0x70300U ; // Enable TX and RX interrupts and DMA requests, address and data
