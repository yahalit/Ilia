...
{
    short unsigned cnt ;
    for ( cnt = 0 ; cnt < (sizeof(SysState.Debug.lDebug) / sizeof(long)) ; cnt++ )
    {
        SysState.Debug.lDebug[cnt] = 0 ;
    }
    for ( cnt = 0 ; cnt < (sizeof(SysState.Debug.fDebug) / sizeof(float)) ; cnt++ )
    {
...
//#pragma FUNCTION_OPTIONS ( ActivateProgrammedRecorder, "--opt_level=0" );
...
long  unsigned   ActivateProgrammedRecorder(void)
{
    ...
    unsigned long  uss1 , uss2;
    short unsigned sr ;

    if ( IsBufferUsedForProgramming() )
    {
        return Manufacturer_error_detail + ERR_RECORDER_BUF_USED  ;
    }

    if ( RecorderProg.RecorderListLen == 0 )
    {
        return Manufacturer_error_detail + ERR_NO_SIGNALS_TO_RECORD ;
    }
    ...
    for ( cnt = 0 ; cnt < RecorderProg.RecorderListLen ; cnt++ )
    {
        ...
        {
            return Manufacturer_error_detail + ERR_RECORDER_BAD_SIGNAL  ;
        }
    }
    ...
    case 4:
        ...
        break ;
    default:
        return Manufacturer_error_detail + ERR_RECORDER_BAD_TRIGGER  ;
    }

    maxlen = REC_BUF_LEN / RecorderProg.RecorderListLen - 1 ;
    uss1 = __lmin( __lmax( RecorderProg.RecLength , 3 ) , maxlen ) ;
    uss2 =  __lmin( RecorderProg.PreTrigCnt,uss1-2) ;
...
long unsigned  GetRecorder( struct CSdo * pSdo ,short unsigned *nData)
{
    ...
    case 100:
        ...
        {
            return Manufacturer_error_detail + ERR_REC_GET_OUT_OF_RANGE  ;
        }
        ...
        {
            return ( Manufacturer_error_detail + ERR_REC_BAD_GETRANGE )  ;
        }
