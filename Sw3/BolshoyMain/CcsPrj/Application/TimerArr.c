...
//Get the IPC timer - its in SYSCLK rate
void GetIpcTimer( unsigned long long *p)
{
    short unsigned mask ;
    mask = BlockInts()  ;
    * (unsigned long *) p  = Cpu1toCpu2IpcRegs.IPCCOUNTERL ;
    ((unsigned long *) p)[1]  = Cpu1toCpu2IpcRegs.IPCCOUNTERH ;
    RestoreInts(mask) ;
}
...
{// RT safe: Timer update in the assembler is in critical section
    ...
    short unsigned mask ;
    long unsigned *p ;
    p = (long unsigned *) &pT->SysTimer ;
    mask = BlockInts() ;
    tmr = ClaState.Timer.UsecTimer  ;
    if ( tmr <  p[0]   )
    {
        p[1] += 1   ;
    }
