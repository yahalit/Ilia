...
long unsigned  SetRecorder( struct CSdo * pSdo ,short unsigned nData)
{
    ...
    if ( pSdo->SubIndex >= 10 && pSdo->SubIndex < (10+N_RECS_MAX ) )
    {
        if ( us < NREC_SIG )
        {
            ...
            ulPtr2 = S2M.Items.Records.buf ;
            RecorderProg.RecorderList[ind] = ulPtr ;

            if ( ( ulPtr >= ulPtr2  ) && ( ulPtr < (ulPtr2+16)  ) )
            {
                RecorderProg.AlienRecorderIndex[ind] = (short unsigned)(ulPtr - ulPtr2)   ;
            }
            else
            {
                RecorderProg.AlienRecorderIndex[ind] = 0 ;
            }
    ...
    case 6: //0 for every interrupt, 1 sync to proc
        ...
        M2S.Items.ActCmd.cmd.ModeWord.bit.RecorderTimeBasis = ( us ? 1 : 0 ) ;
        break;
 ...
 * \brief
 */
float *  GetFSignalPtr( short unsigned si )
{
    struct CCmdMode mode ;
...
long unsigned  GetRecorder( struct CSdo * pSdo ,short unsigned *nData)
{
    ...
    if ( pSdo->SubIndex >= 10 && pSdo->SubIndex < (10+N_RECS_MAX ) )
    {
        ...
        flags = Recorder.RecorderFlags[ind];
         if ( flags & 128 )
        { // Thats an alien
             flags = S2M.Items.Records.flags[RecorderProg.AlienRecorderIndex[ind]];
        }
    ...
    case 110:
        ...
        pSdo->SlaveBuf[RecorderProg.RecorderListLen] = IsrTimer.UsecTimer ;
