...
//#define SIG_TABLE_SIZE_BASIC 200
...
struct CRecorder
{
    ...
    uRecLen PutCntr ; //!< Where to put next record
    //uRecLen GetCntr ; //!< Index of oldest valid record
    short unsigned GapCntr ; //!< decimation counter
    uRecLen StartRec ; //!< The index taken as start of actual record
    uRecLen EndRec   ; //!< The last valid index of the actual record
    uRecLen PreTrigCnt; //!< The (maximal) number of records to place before the trigger event
    uRecLen PreTrigTotalCnt ; //!< The (maximal) number of all-signal-records to place before the trigger event
    ...
    uRecLen RecLength ; //!< Length of a single signal record
    uRecLen TotalRecLength ; // !< Total number of longs that consist a record
    ...
    short unsigned AlienRecorderIndex [N_RECS_MAX];  //!< List of indices for CPU2 flags retrieval
    short TriggerType ; // !< Type of recorder trigger
    short unsigned Sig2Bring ; // !< The signal index to upload
    long  unsigned BringStartIndex ; // !< The first index in the signal to upload
    long  unsigned BringEndIndex ; // !< The last index in the signal to upload
    short unsigned Active ; // !< 1 if recorder had been ever put to action
    short unsigned TriggerIndex ; // !< Index of signal to trigger by
    short unsigned BufferReady ;
    short unsigned Algn  ;
    long FastIntsInC; // !< Fast interrupts in C loop
    long FastTsUsec;  // !< Fast interrupt TS
...
unsigned long volatile StatInSRAMBuffer[1024]; //<! Recorder buffer
#else 
//unsigned long RecorderBuffer[REC_BUF_LEN]; //<! Recorder buffer
__attribute__((far)) unsigned long volatile RecorderBuffer[REC_BUF_LEN]; //<! Recorder buffer
#pragma DATA_SECTION(RecorderBuffer,".farbss");

__attribute__((far)) struct CNavInfo  volatile NavHistoryBuffer[NAV_HIST_BUF_LEN]; //<! Recorder buffer
#pragma DATA_SECTION(NavHistoryBuffer,".farbss");


__attribute__((far)) unsigned long volatile StatInSRAMBuffer[1024]; //<! Recorder buffer
#pragma DATA_SECTION(StatInSRAMBuffer,".farbssS");
#endif
#else
extern const struct CRecorderSig RecorderSigRaw[] ;
extern const short unsigned NREC_SIG ;
...
extern unsigned long  volatile StatInSRAMBuffer[];
#else
extern __attribute__((far)) unsigned long  volatile RecorderBuffer[] ;
extern __attribute__((far)) unsigned long  volatile StatInSRAMBuffer[] ;
extern __attribute__((far)) struct CNavInfo  volatile NavHistoryBuffer[] ;
#endif
