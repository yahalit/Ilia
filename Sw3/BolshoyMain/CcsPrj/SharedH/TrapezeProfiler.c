...
void ReposRotProfiler ( struct CProfiler * pProf , float pos , float maxdelta  , float deltagain )
{
    ...
    delta = __fmax( __fmin( delta, maxdelta ), -maxdelta) * deltagain ;
    ReposProfiler ( pProf , delta ) ;
}


/*
 * \brief Reposition the profiler of an active axis, for linear motion
 * \param pProf-> Profiler to reset
 * \param pos:  now position
 *
 */
void ReposLinProfilerAbsolute ( struct CProfiler * pProf , float pos , float maxdelta  , float deltagain )
{
    float delta ;
    delta = __fmax( __fmin( pos - pProf->ProfilePos , maxdelta ), -maxdelta) * deltagain ;
    ReposProfiler ( pProf , delta ) ;
}

/*
 * Reposition a profiler by a position difference
 * pProf -> Profiler
 * delta : The amount to move
 *
 * Note: If profile reposition takes it over the target, position is set to target and profile is ended.
 */
void ReposProfiler ( struct CProfiler * pProf , float delta  )
{
    short cnt ;
    short unsigned mask ;
    float pos ;
    ...
    mask = BlockInts() ;
    pos   = pProf->ProfilePos + delta   ; // New position

    if ( ( ( pProf->ProfilePos - pProf->PosTarget) * ( pos - pProf->PosTarget)  < 0 ) ||
            ( fabsf ( pos - pProf->PosTarget) < 1e-3f ) )
...
short unsigned  AdvanceProfiler(struct CProfiler * pProf , float dt )
{
    ...
    if ( ( fabsf(v) < 1.6f * pProf->dec * dt)  && (err < 5 * pProf->dec * dt * dt  ) )
    {
        done = 1 ;
    }
    else
    {
       ...
       vm = sqrtf(2.0f * d * err ) ;
        ...
        else
        {
            ...
            if ( ( fabsf(v) < 1.1f * pProf->dec * dt)  && ( fabsf(err) < 1.7f * pProf->dec * dt * dt  ) )
            {
                done = 1 ;
            }
        }
