...
// returns 1 if ManipulatorType != Flex_Manipulator (and more)
short PowerOnSetup(void)
{
    ...
    PState =  PBit.State ;
     ...
     if ( PState < 10 ) //if ManipulatorType != Flex_Manipulator
...
{
    // Accept control word only if motors are not resetting
    ...
    else
    {
        if ( ManipulatorType == SCARA_Manipulator )
        {
            // The DXL does not know to recover servo faults, so on fault it need be completely reset.
            // Accept here a reset command (by increase of StateResetCnt) from core#1
            ...
            SetManipulatorControlWord( M2S.Items.ActCmd.cmd.ControlWord); //get control word from M2S
        }
    }
    ...
    // Run the Scara dynamixel management state machines
