...
void InitManipulatorType(void)
{
    ...
    WheelArmExists   = M2S.Items.ActCmd.cmd.ConfigWord.bit.WheelArmExist ;
...
short unsigned ArePumpsOn (void)
{
    ...
    {
        return SysState.EEF.ControlWord.bit.ActivatePackageGrip;
    }
    else
    {

        return 0; //return M2S.Items.ActCmd.cmd.PumpsOn; //TODO:SACARA fix return value if using SACARA. returning an M2S value is wrong.
    }
}
...
void main(void)
{
    ...
    while(1)
    {
        S2M.Items.ActStatus.stat.LifeTracker = M2S.Items.ActCmd.cmd.LifeCheck ;
        IdleTask();
    }
...
void IdleTask (void)
{
    ...
    //CAN1 - Review new message from M2S, if not read then run set/ get functions based on the object dictionary
    ObjectSlave() ;

    if (FlexManipControl.PerformHomingFromIdleLoop)
    {
        FlexHoming();
    }

    IdleCbit() ; //Analyze system state, BIT - built in test, CBit - continuous bit
    IdleCbitResponse() ; // React system CBIT findings, Dynamixel only

    if (IsPsActive() ) // 36 V is stable /
    {
        //if ( PowerOnSetup() )
        if ( PowerOnSetup() ) //TODO: show Yahali, the PowerOnSetup() returns 0 even with rigid arms and Flex.
        {
            RunDynamixelSystem();
    ...
    IdleCanProc() ; // CAN2 - check for new incoming SDO from slaves (PDU & CAN_SLAVES SDO and boot-up) and search for its traps (non Object dictionary SDO). PDU communications need be considered even when axes are no ready.
...
short OperSyncReport( void )
{
    ...
    //S2M.Items.ActStatus.stat.LaserVolts = SysState.EEF.LaserVolts ; //moved from Core2CanHandlerTx, LaserVolts removed from S2M by Yahali.
    ...
    if ( Dynamixel12.nMembers == 0 )
    {
        uBit.s.ManCBit.Dyn12InitDone = 0 ;
        uBit.s.ManCBit.Dyn12NetOn =  0 ;
    }
    if ( Dynamixel24.nMembers == 0 )
    {
        uBit.s.ManCBit.Dyn24InitDone = 0 ;
        uBit.s.ManCBit.Dyn24NetOn =  0 ;
    }

    uBit.s.ManCBitH = ManCBitH ;

    S2M.Items.ActStatus.stat.ManipData1_L = uBit.ul ;
    S2M.Items.ActStatus.stat.SlaveStat.bit.hi.Homed = FlexManipControl.AxesHomed;

    uPtr[1] = SysState.Mot.mode ; //0 ;
    ...
    {
        S2M.Items.ActStatus.stat.ManipulatorX = ManCmd.x;
        S2M.Items.ActStatus.stat.ManipulatorY = ManCmd.y;
        S2M.Items.ActStatus.stat.ManipulatorAngleRad = ManCmd.theta;
    }
   ...
   //uPtr[2] = (short) ( mod2piS (ManCmd.theta)  * 10430.0F ) ;
    ...
    sPtr[1]  = (short) ( __fmax( __fmin( y1 * 1000 , 32700 ) , -32700  )  ) ; //32700 clipping if number is too high for short int.
...
long unsigned LogException ( short unsigned ID ,  short fatality , long unsigned exp )
{
    ...
    if ( fatality == EXP_FATAL )
    {
        ...
        SetMotionMode(E_SysMotionModeFault) ;//by itself this state send motor of to all axes.
    }
    /*
    ...
    */
    ...
    { // The ABORT handles system errors that are NOT servo failures. It aborts programmed actions
        ...
        {//ManipulatorType == Flex_Manipulator
            FlexManipAxesStopMan();
            FlexManipControl.State = MAN_ST_FAILURE;
            FlexManipControl.GoTo.IsActive  = 0 ; //stops ManagePosture (used in go to StandBy and in DealPackge).
            FlexManipControl.ControlWordCopy.Standby = 0; //stops standBy process.
            FlexManipControl.ControlWordCopy.DealPackage = 0; //stops deal package process.
            FlexManipControl.ControlWordCopy.DoHoming = 0; //stops homing process.
...
void InitSysData(void)
{
    ...
    ClearMemRpt ( (short unsigned *) &FlexManipControl, (short unsigned) sizeof(  FlexManipControl  ) )  ;
 ...
 //   NUM_MAN_AXES = DEF_NUM_MAN_AXES ;
    ...
    SysState.CanAxis[LOGICAL_PLATE].BH.CurrentUseRatio = TAPE_ARM_DEFAULT_CURRENT_USE_RATIO ;

    SysState.EEF.EEFInstalled = InstalledEEF;
    ...
    SysState.PosConvTolPlate = AxisTolerancePlate *1.2f;  //*2 ;//+ 0.002 //TODO:CONVERGENCE optimize;
    SysState.PosConvTolTape = AxisToleranceTape * 1.1f;  //1.5 ; //TODO:CONVERGENCE optimize;
    SysState.PosConvTolSpacer = AxisToleranceShifter * 1.1f;  //*1.5 ; //TODO:CONVERGENCE optimize;

    SysState.PosCompTolPlate = AxisTolerancePlate *1.5f;  //*5 ;//+ 0.002 // *1.3 would do.
    SysState.PosCompTolTape = AxisToleranceTape *3.0f;  //*5 ; // when the brake engage and the motor shut doown the axis probably move a bit and then the command gets the current position.
    SysState.PosCompTolSpacer = AxisToleranceShifter *1.5f;  //*5 ; // *1.3 would do.

    for ( cnt = 0 ; cnt < 4 ; cnt++ )
    {
        SysState.AxisWakeUpTryCnt[cnt] = 0;
    }

    SysState.CurrentInitialSetPointTol = 0.025; //about 1.43 deg
    SysState.EEF.sentCmdCounter = 0;
    FlexManipControl.NewCommand = 0;
    FlexManipControl.NewCwFromManCmd = 0;
    FlexManipControl.CWCounter = 0;
    FlexManipControl.numOfLastSegment = numOfPointsOnTrack - 2;
    FlexManipControl.LinearHomigPositionFW = LinearHomigPositionFWConst;
    FlexManipControl.LinearHomigPositionRev = LinearHomigPositionFWConst;
    FlexManipControl.CwOwner = 1;
    FlexManipControl.postureCmdOD = 0;
    FlexManipControl.PostureFix = 0;
    FlexManipControl.PerformHomingFromIdleLoop = 0;

    TrayShelfTrackPnt [0].angle =      ManGeo.ShelfTrackPoint1Angle;
    TrayShelfTrackPnt [0].position =   ManGeo.ShelfTrackPoint1Pos;
    //TrayShelfTrackPnt [0].PrevSegDirIsRot= -1;
    //TrayShelfTrackPnt [0].NextSegDirIsRot= 0;
    TrayShelfTrackPnt [1].angle =      ManGeo.ShelfTrackPoint2Angle;
    TrayShelfTrackPnt [1].position =   ManGeo.ShelfTrackPoint2Pos;
    //TrayShelfTrackPnt [1].PrevSegDirIsRot= 0;
    //TrayShelfTrackPnt [1].NextSegDirIsRot= 1;
    TrayShelfTrackPnt [2].angle =      ManGeo.ShelfTrackPoint3Angle;
    TrayShelfTrackPnt [2].position =   ManGeo.ShelfTrackPoint3Pos;
    //TrayShelfTrackPnt [2].PrevSegDirIsRot= 1;
    //TrayShelfTrackPnt [2].NextSegDirIsRot= 0;
    TrayShelfTrackPnt [3].angle =      ManGeo.ShelfTrackPoint4Angle;
    TrayShelfTrackPnt [3].position =   ManGeo.ShelfTrackPoint4Pos;
    //TrayShelfTrackPnt [3].PrevSegDirIsRot= 0;
    //TrayShelfTrackPnt [3].NextSegDirIsRot= 1;
    TrayShelfTrackPnt [4].angle =      ManGeo.ShelfTrackPoint5Angle;
    TrayShelfTrackPnt [4].position =   ManGeo.ShelfTrackPoint5Pos;
    //TrayShelfTrackPnt [4].PrevSegDirIsRot= 1;
    //TrayShelfTrackPnt [4].NextSegDirIsRot= 0;

    SysState.PdProcCtrMax = 2 ;
...
void IdleCBitScaraManip()
{
    SysState.Pump.On = ArePumpsOn();
    if (SysState.Pump.On)
    {
        SetSysTimerTargetSec(TMR_WAIT_SUCK_DEPLETE,
                             ManGeo.TimeForEntireSuckRelease, &SysTimerStr);
    }
    SysState.Pump.IsDepleted =
            IsSysTimerElapse(TMR_WAIT_SUCK_DEPLETE, &SysTimerStr) ? 1 : 0; //the timer will keep restarting as long as the pumps are on.
    if (SysState.Package.LaserMedianValid)
    {
        ManCBit.NoSuck1 = (SysState.Package.LaserDistMedian
                > ManGeo.MaxLaserDistOnHold);
        SetSysTimerTargetSec(TMR_WAIT_LASER_INVALID, 0.3f, &SysTimerStr);
    }
    else
    {
    ...
    {
        SetSysTimerTargetSec(TMR_WAIT_SUCK_HOLD, ManGeo.TimeInGoodCurrent4Pull,
                             &SysTimerStr);
    }
    SysState.Pump.IsHolding =
            IsSysTimerElapse(TMR_WAIT_SUCK_HOLD, &SysTimerStr) ? 1 : 0;
    ApplyScaraLaser();
 ...
 * Test sucking state, test Dynamixel, applies laser, checks package manipulator wellness.
 */
void IdleCbit(void)
{
    short unsigned mask ;
    ...
    {
        Flash_RelinquishPumpSemaphore() ; //declares that Core2 is done using the flash programming internal power supply.
    ...
    ManCBitH.IndividualAxControl = SysState.Mot.mode == E_SysMotionModeIndividual ? 1 : 0 ;
    ...
    S2M.Items.ActStatus.stat.SlaveStat.bit.hi.Individual = SysState.Mot.mode==E_SysMotionModeIndividual ? 1 : 0;

    if ( EEFRxPdoBitStr.ErrorCount > EEFRxPdoBitStr.MaxCount) 
    {
        ...
        SetFlexManipError ( exp_EEF_Max_Allowed_PDOTx_Missing , ERROR_SRC_SYS , OWN_EXP_ID, EXP_ABORT ) ;
    }

    if (( SysState.PsWakeState != SYSSTAT_WAKE_WAIT_OPER  ) && SysState.PsActivationCtr )
    {
        SysState.PsActivationCtr  |= 0x40 ;
    }

    mask = BlockInts() ;
    if (( S2M.Items.ActStatus.stat.PsStatus.bit.PsWakeState == SYSSTAT_WAKE_WAIT_OPER) && (SysState.PsWakeState != SYSSTAT_WAKE_WAIT_OPER))
    {
        SysState.PsActivationCtr  |= 0x80 ;

    }
    S2M.Items.ActStatus.stat.PsStatus.bit.PsWakeState  = SysState.PsWakeState   ;
    S2M.Items.ActStatus.stat.PsStatus.bit.ManOnState   = SysState.ManOnState ;
    RestoreInts(mask) ;

}

/**
 * \brief respond to the findings of self test (Dynamixel only)
 */
