...
struct CGoToState //set by GoToPosture
{
    float length ;  //final GoToPosture tape arm target (managed in ManagePosture).
    float angle  ;  //final GoToPosture rotation (plate) target (managed in ManagePosture).
    float Linear  ; //final GoToPosture linear (spacer) target (managed in ManagePosture).
    float nextLinearCmd ; //temporary linear (spacer)  target set in ManagePosture.
    float nextPlateCmd ; //temporary plate (rotation) target set in ManagePosture.
    short unsigned Flags ; //FLAG_POST_SEEPACK 1 , FLAG_POST_SUCK_TEST 2 , FLAG_POST_SLOW 4 , FLAG_POST_AUTO_LINEAR 8 , FLAG_POST_ROBOT_COORD 16
    short Error  ;
    short State  ;
    short IsActive ; //when FlexManipAxes.GoTo.IsActive > 0 then the FlexManipHandleOperations will run ManagePosture continously until done (and not enter getPackage/ setPacakge) until done.
};

struct CSuckRequest
{
    short unsigned DoSuck : 1 ; // Command to perform sucking
    short unsigned DoNotTestSuction : 1 ; // Command not to test actual suction, used for continuous suctino.
    short unsigned RestartTimerAnyway  : 1 ; // Used to restart timer anyway, not used for Flex.
} ;
...
//{
//    short PackageGet ;  // 1 -Package GET command/ 0 - Package Set command
//    short IsRepeated ; // Is repeated access attempt
//    short Side; //package Side
//    short DealPackage; // pending DealPackage command
//    short StandBy; //pending standBy command
//    short DoHoming; //pending do homing command.
//    short KillHoming; // !< 1 to kill an existing homing
//};
...
struct CManCmd
{
    ...
    short GetSide; //!< The side to get the package, can be -1:left or 1:right. set in ProgramFlexManip() within SetManipulatorControlWord() based on M2S.Items.ActCmd.cmd.AssumedPackagePositionY, M2S.Items.ActCmd.cmd.AssumedPackagePositionX.
    ...
    float x ; // !< End effector suction cups center coordinate x
    float y ; //End effector suction cups center coordinate y
    float theta ; //End effector suction cups center coordinate Theta
    float Ex ; // !< x of elbow
    float Ey ; // !< y of elbow
    ...
    short unsigned StdbyRequest ; // !< Request to stand by. //Relevant only to Scara
    ...
    short unsigned RdyNewAction ; // !< Flag that motion is properly programmed and can be executed. bits 0-1 (values 1/2) Each bit manifests that another motion property is programmed
                                    //  4: New action is conditioned on the data being programmed
...
struct CManGeo
{
    ...
    float FlexStandbyExtPos; // Standby target for traverse position
...
//    float FlexDiscSpeed   ; // Max speed for Flex disc
    ...
    float FlexArmRetreatDist4SuctRelease; // // Flex arm retreat for pressure release
    float FlexMaxExtensionTravel   ; // Maximum flex arm travel (max position)
    float FlexMaxNeckIncidence;
    float FlexMaxAngle   ; // Maximum flex arm angle
    float FlexMaxLinearTravel ; // Maximum linear axis position
    float FlexXDistWheel2PackageAt90DegPlate ; // !< Distance on the X axis between the wheel center and tape arm at spacer 0
    float FlexYDistWheel2PackageAtSpacer0Ext0 ; // !< Distance on the y axis between the wheel center and tape arm at spacer 0 //TODO:INTEGRATION check
    float FlexArmSuspectedPackageContactThreshold; // !< Flex Arm Suspected Package Contact Threshold
    float FlexSpacerSpeed    ; // Homing speed for flex arm spacer
    float FlexPlateSpeed ; // Speed for manipulator rotator
    float FlexTapeSpeed ; // Speed for manipulator tape
    float FlexSpacerAcc    ; // acceleration speed for flex arm spacer
    float FlexPlateAcc ; // acceleration for manipulator rotator
    float FlexTapeAcc ; // acceleration for manipulator tape
    float FlexStandbySpacerPos ; //  Flex arm spacer operational position (10mm smaller that FlexSpacerStandbyPos, probably can be equal to it).
    //float FlexCupsRel2RotaionCenter; // suction cups center relative to rotation center while Tape arm at zero position (at switch). positive is behind the center.
    float FlexSpacerPosSetPackageShelf;
    float FlexArmZeroY2CupsAtSpacer0Ext0; //Flex Arm 0Y 2 Cups At Spacer zero position
    //float FlexArmDistLaser2CupsUnPressed; //Flex Arm Dist Laser 2 Cups
    float FlexArmDistLaser2CupsPressed; //Flex Arm Dist Laser 2 Cups
    float FlexArmMaxExtWithinTray;  //Rough Approach Clearance From Expected Package (fine approach required)
    float FlexArmActivateSlowApproach; //the distance from package where need to start slow approach.
    float FlexArmPackageGrippedLaserTol; //flex arm tolerance for laser measurement result for package gripped
    float FlexArmPackageOnTrayLaserTol; //flex arm tolerance for laser measurement result for package on tray
    float FlexArmExtUpdateTol; //the required change in laser reading to update the get package fine approach Y target
    float FlexArmETolForPackageY; // Tolerance allowed for for Package Y location difference between expected Y and measured Y
    float FlexArmPackagePreloadExt; //define how much to advance over the current D target in order to pre-load the suction cups

    float ShelfTrackPoint1Angle;
    float ShelfTrackPoint1Pos;
    float ShelfTrackPoint2Angle;
    float ShelfTrackPoint2Pos;
    float ShelfTrackPoint3Pos;
    float ShelfTrackPoint3Angle;
    float ShelfTrackPoint4Pos;
    float ShelfTrackPoint4Angle;
    float ShelfTrackPoint5Pos;
    float ShelfTrackPoint5Angle;
};
MAN_EXTERN_TAG struct CManGeo ManGeo ; //Set in ParRecords2.h
...
{
    float LineSpeed ;
    unsigned short SuctionOn; //for flex - sets SysState.Package.SuckRequest.items.DoSuck. If also used SetEEFManual then OverRideSwitches is set and ManualManCmd.SuctionOn is ignored.
