 ...
 */
void SetManipulatorControlWord( short unsigned cw_in  )
{
    union
    {
    ...
    if (  ( ManCmd.ControlWord.MotorsOn == 0 ) && cw.cw.MotorsOn )
    {
        if ( ( (ManCmd.RdyNewAction == 3 ) && (cw.cw.Package) ) ||  ( (ManCmd.RdyNewAction == 4 ) && (cw.cw.Standby) ) || ((cw.cw.Package==0)&&(cw.cw.Standby==0)) )
        {
...
{
    short dir ;
    switch ( ManCmd.ControlWordCopy.Side )
    {
...
void ManageAutomaticMotorOnOff()
{
    ...
    else
    {
        ...
        if ( (ManCmd.ControlWord.Package == 0 ) && (ManCmd.ControlWord.Standby == 0 )  && (ManCmd.ControlWord.DoHoming == 0 ) )
        {
            SetSysTimerTargetSec ( TIMER_DYNAMIXEL_ON_TMR12 , DYNAMIXEL_MOTOR_ON_TOUT ,&SysTimerStr );
            for ( cnt = 0 ; cnt < NUM_12V_MANIP_AXES ; cnt++ )
            {
...
void CalcAutoManipulatorState24( float  dT , short unsigned updAxis  )
{
    ...
    if (  ( ManCmd.ControlWord.Standby == 0) && (ManCmd.ControlWord.Package == 0) && (ManCmd.ControlWord.DoHoming == 0 ) )
    {
        SysState.Package.LaserRequest =  0 ;
        ...
        { // Enough time already in place
            ...
            if (  IsRepush || SysState.Package.LaserDistValid)
            {
                ...
                else
                {
                    ...
                    InitSuctionImmediate() ; // Apply suck for the push, no test
        ...
        { // Enough time already in place
            ...
            if ( SysState.Package.LaserDistValid  )
            {
               ...
               else
               {
                   ...
                   InitSuctionImmediate()  ; // Liran asked to put suction at the entire final approach
