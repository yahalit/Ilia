...
#else
#define REC_BUF_LEN 6144 // Number of bins in recorder
#endif 
...
struct CRecorder
{
	...
	short GapCntr ; //!< decimation counter
	...
	uRecLen BufSize ; //!< (not necessarily) 2^N-1, where the buffer length is 2^N longs
	...
	short unsigned BringStartIndex ; // !< The first index in the signal to upload
	short unsigned BringEndIndex ; // !< The last index in the signal to upload
...
REC_EXTERN_VAR struct CRecorder2 Recorder2;
REC_EXTERN_VAR struct CRecorder Recorder;
REC_EXTERN_VAR struct CRecorder RecorderProg;
REC_EXTERN_VAR struct CRecorder2 RecorderProg2;
